...
public class Game {
	...
	public Game(int width, int height, Cell[] data, Cursor cursor, int clicks, String name, LocalDateTime start_time) {
		this.width = width;
		this.height = height;
		this.data = data;
		this.cursor = cursor;
	...
	public static Optional<Game> fromSave(String name) {
		...
		try {
			...
			ans = Optional
					.of(new Game(width, height, data.toArray(Cell[]::new), new Cursor(0, 0), 0, name, start_time));
	...
	public Game up() {
		return new Game(this,
				new Cursor(clamp(this.cursor.x(), 0, this.width - 1), clamp(this.cursor.y() - 1, 0, this.height - 1)));
	}

	public Game left() {
		return new Game(this,
				new Cursor(clamp(this.cursor.x() - 1, 0, this.width - 1), clamp(this.cursor.y(), 0, this.height - 1)));
	}
	...
	public Game confirm() {
		...
		var ans = new Game(this, new_cell, this.cursor.x(), this.cursor.y(), this.clicks + 1);
		ans.save();
		return ans;
	}

	/*
	 * public Game check(String name) { boolean collum = false; int collum =
	 * this.width; int y = 0; var original = //was einsetzten this.data?; var
	 * Filename = name; var solved = Game.fromSave(Filename);
	 * 
	 * while(y < collum) { if(original.equals(solved.get())) { collum = true; }
	 * if(numbers count){ collum = true; } else { collum = false; }
	 * 
	 * y++; collum = collum + this.width; } while
	 */
	...
	public String toHtml(String base_route) {
		var up_div = new Div("")
				.htmx(new HTMX().addTrigger(HTMXTrigger.KeyUp).post(base_route + "/up").target_id("board"));
		var left_div = new Div("")
				.htmx(new HTMX().addTrigger(HTMXTrigger.KeyLeft).post(base_route + "/left").target_id("board"));
		var right_div = new Div("")
				.htmx(new HTMX().addTrigger(HTMXTrigger.KeyRight).post(base_route + "/right").target_id("board"));
		var down_div = new Div("")
				.htmx(new HTMX().addTrigger(HTMXTrigger.KeyDown).post(base_route + "/down").target_id("board"));
		var mark_div = new Div("")
				.htmx(new HTMX().addTrigger(HTMXTrigger.KeySpace).post(base_route + "/mark").target_id("board"));
		var confirm_div = new Div("")
				.htmx(new HTMX().addTrigger(HTMXTrigger.KeyEnter).post(base_route + "/confirm").target_id("board"));
		var paragraph = new Paragraph(this.innerHtml()).id("board").border(new Border(1, WidthUnits.EM))
				.fontFamily(FontFamily.MONOSPACE);
		return paragraph.toHtml() + confirm_div.toHtml() + mark_div.toHtml() + up_div.toHtml() + left_div.toHtml()
				+ right_div.toHtml() + down_div.toHtml();
	}

	/**
	 * equality checks the width, height, name and field data for equality. it
	 * doesn't consider cursor position or clicks
	 *
